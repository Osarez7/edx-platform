...
class CachingDescriptorSystem(MakoDescriptorSystem):
    ...
    def _load_item(self, usage_id, course_entry_override=None):
        ...
        if isinstance(usage_id, LocalId):
            try:
    ...
    def xblock_from_json(self, class_, usage_id, json_data, course_entry_override=None):
        ...
        try:
            module = self.construct_block_from_class(
                class_,
                field_data,
                ScopeIds(None, json_data.get('category'), definition_id, usage_id)
            )
        except Exception:
        ...
        if isinstance(usage_id, LocalId):
            self.local_modules[usage_id] = module
